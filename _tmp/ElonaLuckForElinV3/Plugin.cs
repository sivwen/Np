...
public sealed class Plugin : BaseUnityPlugin
{
    ...
    internal static ConfigEntry<bool> DropPatch=null!,Corpse=null!,Gene=null!,Materials=null!,UniqueLoot=null!,CombatLoot=null!,StealWeight=null!;
    ...
    internal static ConfigEntry<int> CombatLuckDiv=null!,CombatLuckCap=null!,CritBonus=null!,ExecutionerBonus=null!,OverkillCap=null!,CombatTotalCap=null!;
    ...
    void Awake()
    {
        ...
        DropPatch=Config.Bind("드롭 운","v3 드롭 패치 사용",true,"SpawnLoot의 개별 판정식만 좁게 수정합니다. 패턴이 다르면 드롭 패치를 자동 건너뜁니다.");
        Corpse=Config.Bind("드롭 운","시체 해부학+운",true,"SpawnLoot 내부 해부학 판정에서만 운을 보조값으로 사용합니다.");
        Gene=Config.Bind("드롭 운","유전자 해부학+운",true,"원래 유전자 판정 분모만 해부학+운으로 완화합니다.");
        Materials=Config.Bind("드롭 운","일반 소재 드롭 운",true,"송곳니/가죽/내장/심장/기계부품/골렘 결정의 기존 판정만 완화합니다.");
        UniqueLoot=Config.Bind("드롭 운","몬스터 고유 드롭 운",true,"sourceCard/race 고유 loot의 원래 rnd(1000) 판정만 운으로 보정합니다.");
        CombatLoot=Config.Bind("드롭 운","전투 마무리 장비/소지품 드롭",true,"몬스터 장비/소지품 판정에 운·실제 크리티컬·처형자·오버킬을 반영합니다.");
        AnatomyW=Config.Bind("드롭 운","해부학 가중치",3,"시체/유전자 혼합값에서 해부학의 가중치입니다.");
        AnatomyLuckW=Config.Bind("드롭 운","운 가중치",2,"시체/유전자 혼합값에서 운의 가중치입니다.");
        GeneDiv=Config.Bind("드롭 운","유전자 보너스 강도 분모",2,"해부학+운 혼합값을 이 값으로 나눈 %만큼 유전자 상대 확률을 높입니다.");
        ...
        UniqueDiv=Config.Bind("드롭 운","고유 드롭 운 분모",10,"운이 이 값만큼 오를 때 몬스터 고유 드롭 상대 확률이 1% 증가합니다.");
        ...
        StealWeight=Config.Bind("훔치기 운","중량 제한 운 우회",true,"AI_Steal의 tooHeavy 판정에서만 운으로 중량 제한을 확률적으로 우회합니다. 훔치기 시도마다 새로 굴립니다.");
        ...
        SeedLuck=Config.Bind("채집/농사 운","씨앗 회수 운",true,"TryPopSeed의 원래 수동 씨앗 회수 RNG 분모만 운으로 완화합니다.");
        ...
        TreasureLuck=Config.Bind("보물상자 운","보물 장비 희귀도 운",true,"CreateTreasureContent의 로컬 SetRarity RNG만 운으로 보정합니다.");
        ...
        ScratchLuck=Config.Bind("스크래치 운","스크래치 당첨 운",true,"TraitCrafter의 로컬 Prize() RNG 분모만 운으로 완화합니다. 기존 상품 우선순서를 그대로 유지합니다.");
        ...
        FertEggLuck=Config.Bind("생산 운","수정란 운",true,"플레이어 진영 Card.MakeEgg의 원래 fertChance 분모만 운으로 완화합니다.");
        ...
        h=new Harmony(G);
        h.PatchAll();
        Logger.LogInfo(N+" "+V+" loaded. Card.Die/전역 RNG 패치 없음. 개별 판정만 좁게 패치.");
    }
    ...
    internal static double Curve(double x){if(x<=0)return 0;if(x<10)return x/10*.15;if(x<40)return .15+(x-10)/30*.35;if(x<100)return .5+(x-40)/60*.25;if(x<200)return .75+(x-100)/100*.25;return 1+(x-200)/100;}
...
static class WitnessPatch
{
    static void Prefix(Chara criminal,ref Func<Chara,bool> funcWitness)
    {
        ...
        if(a<=0)return;
        var old=funcWitness;
        if(old==null)funcWitness=c=>EClass.rnd(10)==0&&EClass.rnd(100)>=a;
        else funcWitness=c=>old(c)&&EClass.rnd(100)>=a;
    }
}
...
static class LockPatch
{
    ...
    {
        __state=new S();
        if(!Plugin.Lock.Value||cc==null||!cc.IsPC||__instance.owner==null)return;
        int cut=Math.Min(Plugin.LockCap.Value,Math.Max(0,Plugin.Luck()/Math.Max(1,Plugin.LockDiv.Value)));
        __state.lv=__instance.owner.c_lockLv;
        int n=Math.Max(0,__state.lv-cut);
        if(n<__state.lv){__instance.owner.c_lockLv=n;__state.changed=true;}
    }
    static Exception? Finalizer(Trait __instance,S __state,Exception? __exception)
    {
        if(__state!=null&&__state.changed&&__instance.owner.c_lockLv>0)__instance.owner.c_lockLv=__state.lv;
        return __exception;
    }
}
...
static class FishPatch
{
    ...
    {
        if(!Plugin.Fish.Value||c==null||!c.IsPC||__result==null||__result.category==null||!__result.category.IsChildOf("fish"))return;
        int a=Math.Min(Plugin.FishCap.Value,Math.Max(0,Plugin.Luck()/Math.Max(1,Plugin.FishDiv.Value)));
        if(__result.tier<3&&EClass.rnd(100)<a)__result.SetTier(Math.Min(3,__result.tier+1));
    }
}

[HarmonyPatch(typeof(Map),nameof(Map.TrySmoothPick),new Type[]{typeof(Point),typeof(Thing),typeof(Chara)})]
static class ActivityPatch
{
    static void Prefix(Thing t,Chara c)
    {
        if(!Plugin.Activity.Value||t==null||c==null||!c.IsPC)return;
        var fs=new StackTrace(1,false).GetFrames();if(fs==null)return;
        int skill=-1;bool on=false;
        for(int i=0;i<fs.Length&&i<12;i++)
        {
            string dn=fs[i].GetMethod()?.DeclaringType?.FullName??"";
            if(dn.Contains("TaskMine")){skill=c.Evalue(220);on=Plugin.Mine.Value;break;}
            if(dn.Contains("TaskDig")){skill=c.Evalue(230);on=Plugin.Dig.Value;break;}
            if(dn.Contains("TaskChopWood")){skill=c.Evalue(225);on=Plugin.Harvest.Value;break;}
            if(dn.Contains("TaskHarvest")||dn.Contains("GrowSystem")){skill=Math.Max(c.Evalue(250),c.Evalue(286));on=Plugin.Harvest.Value;break;}
        }
        if(!on||skill<0)return;
        int r=Plugin.Rolls(skill);if(r>0){int b=Math.Max(1,t.Num);t.ModNum(b*r);}
    }
}

[HarmonyPatch(typeof(AI_Fish),nameof(AI_Fish.Makefish),new Type[]{typeof(Chara)})]
static class FishBonusPatch
{
    static void Postfix(Chara c,ref Thing __result)
    {
        if(!Plugin.Activity.Value||!Plugin.FishBonus.Value||c==null||!c.IsPC||__result==null)return;
        int r=Plugin.Rolls(c.Evalue(245));if(r>0){int b=Math.Max(1,__result.Num);__result.ModNum(b*r);}
    }
}

static class CraftCore
{
    internal static List<Thing>? Prep(Recipe r,List<Thing> ings,bool model)
    {
        if(!Plugin.Activity.Value||!Plugin.Craft.Value||model||r==null||ings==null||EClass.pc==null)return null;
        Element e=r.source?.GetReqSkill();int rr=Plugin.Rolls(e==null?0:EClass.pc.Evalue(e.id),true);if(rr<=0)return null;
        var list=new List<Thing>();foreach(var x in ings)if(x!=null)list.Add(x.Duplicate(Math.Max(1,x.Num)*rr));return list;
    }
    internal static void Give(List<Thing>? xs){if(xs==null||EClass.pc==null)return;foreach(var x in xs)EClass.pc.AddCard(x);}
}
[HarmonyPatch(typeof(Recipe),nameof(Recipe.Craft),new Type[]{typeof(BlessedState),typeof(bool),typeof(List<Thing>),typeof(TraitCrafter),typeof(bool)})]
static class RecipePatch{static void Prefix(Recipe __instance,List<Thing> ings,bool model,out List<Thing>? __state){__state=CraftCore.Prep(__instance,ings,model);}static void Postfix(List<Thing>? __state,Thing __result){if(__result!=null)CraftCore.Give(__state);}}
[HarmonyPatch(typeof(RecipeCard),nameof(RecipeCard.Craft),new Type[]{typeof(BlessedState),typeof(bool),typeof(List<Thing>),typeof(TraitCrafter),typeof(bool)})]
static class RecipeCardPatch{static void Prefix(RecipeCard __instance,List<Thing> ings,bool model,out List<Thing>? __state){__state=CraftCore.Prep(__instance,ings,model);}static void Postfix(List<Thing>? __state,Thing __result){if(__result!=null)CraftCore.Give(__state);}}
...
static class CasinoPatch
{
    ...
    {
        if(!Plugin.Casino.Value||__instance?.balance==null||__instance.balance.changeCoin<=0)return;
        int a=Math.Min(Plugin.CasinoCap.Value,Math.Max(0,Plugin.Luck()/Math.Max(1,Plugin.CasinoDiv.Value)));
        if(EClass.rnd(100)<a)__instance.balance.changeCoin+=Math.Max(1,__instance.balance.changeCoin/2);
    }
}
...
static class DropMath
{
    ...
    internal static int AdjustChance(int denominator,Card victim,Card origin,int kind)
    {
        ...
        if(kind==1&&Plugin.Gene.Value){int anatomy=origin==null?0:Anatomy(origin,290);bonus=Math.Min(Plugin.GeneCap.Value,Math.Max(0,anatomy/Math.Max(1,Plugin.GeneDiv.Value)));}
        else if(kind==2&&Plugin.Materials.Value)bonus=Math.Min(Plugin.MaterialCap.Value,Math.Max(0,Plugin.Luck()/Math.Max(1,Plugin.MaterialDiv.Value)));
        else return denominator;
        return Plugin.ReduceDenom(denominator,bonus);
    }
    internal static int UniqueRnd(int max)
    {
        if(!Plugin.DropPatch.Value||!Plugin.UniqueLoot.Value||max<=1)return EClass.rnd(max);
        int bonus=Math.Min(Plugin.UniqueCap.Value,Math.Max(0,Plugin.Luck()/Math.Max(1,Plugin.UniqueDiv.Value)));
        return EClass.rnd(Plugin.ReduceDenom(max,bonus));
    }
    internal static int CombatBonus(Card victim,Card origin)
    {
        ...
        int b=Math.Min(Plugin.CombatLuckCap.Value,Math.Max(0,Plugin.Luck()/Math.Max(1,Plugin.CombatLuckDiv.Value)));
        var ap=AttackProcess.Current;if(ap!=null&&ap.CC==origin&&ap.TC==victim&&ap.crit)b+=Math.Max(0,Plugin.CritBonus.Value);
    ...
    internal static int AdjustCombatDenom(int denominator,Card victim,Card origin){return Plugin.ReduceDenom(denominator,CombatBonus(victim,origin));}
    internal static float CombatRndf(float max,Card victim,Card origin){if(max<=0)return EClass.rndf(max);int b=CombatBonus(victim,origin);if(b<=0)return EClass.rndf(max);return EClass.rndf(max)/(1f+b/100f);}
}

[HarmonyPatch(typeof(Card),nameof(Card.SpawnLoot),new Type[]{typeof(Card)})]
static class SpawnLootNarrowPatch
{
    ...
    static bool IsCall(CodeInstruction c,string name)=>c.operand is MethodInfo m&&m.Name==name&&(c.opcode==OpCodes.Call||c.opcode==OpCodes.Callvirt);
    static bool IsChance(CodeInstruction c)=>c.operand is MethodInfo m&&m.Name.Contains("g__chance")&&(c.opcode==OpCodes.Call||c.opcode==OpCodes.Callvirt);
    static bool Ldc(CodeInstruction c,int v){if(c.opcode==OpCodes.Ldc_I4)return c.operand is int x&&x==v;if(v==-1)return c.opcode==OpCodes.Ldc_I4_M1;if(v>=0&&v<=8)return c.opcode==new[]{OpCodes.Ldc_I4_0,OpCodes.Ldc_I4_1,OpCodes.Ldc_I4_2,OpCodes.Ldc_I4_3,OpCodes.Ldc_I4_4,OpCodes.Ldc_I4_5,OpCodes.Ldc_I4_6,OpCodes.Ldc_I4_7,OpCodes.Ldc_I4_8}[v];if(c.opcode==OpCodes.Ldc_I4_S)return Convert.ToInt32(c.operand)==v;return false;}
    static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        if(chance.Count<14||anatomy.Count<2){Plugin.I.Logger.LogWarning($"v3 SpawnLoot 패턴 불일치: chance={chance.Count}, anatomy={anatomy.Count}. 원본 유지.");foreach(var x in src)yield return x;yield break;}
        ...
        if(unique<0||rndf<0||eq100<0||item5<0){Plugin.I.Logger.LogWarning("v3 SpawnLoot 세부 패턴 불일치. 원본 유지.");foreach(var x in src)yield return x;yield break;}
        var anatomySet=new HashSet<int>(anatomy);var chanceKind=new Dictionary<int,int>{{chance[2],1}};for(int n=3;n<=9;n++)chanceKind[chance[n]]=2;for(int n=11;n<=13;n++)chanceKind[chance[n]]=2;
        ...
        Plugin.I.Logger.LogInfo("v3 SpawnLoot 좁은 패치 적용 완료.");
    }
}

static class StealAttemptContext
{
    [ThreadStatic] internal static int roll10000;
    [ThreadStatic] internal static bool hasRoll;
}

[HarmonyPatch(typeof(AI_Steal),nameof(AI_Steal.Perform),new Type[]{})]
static class StealAttemptRollPatch
{
    static void Prefix(){StealAttemptContext.roll10000=EClass.rnd(10000);StealAttemptContext.hasRoll=true;}
}
...
static class StealWeightNarrowPatch
{
    ...
    {
        int found=0;foreach(var t in Nested(typeof(AI_Steal)))foreach(var m in t.GetMethods(BindingFlags.Instance|BindingFlags.Static|BindingFlags.Public|BindingFlags.NonPublic)){if(m.IsAbstract||m.ContainsGenericParameters)continue;if(CallsGetter(m)){found++;yield return m;}}
        if(found==0&&Plugin.I!=null)Plugin.I.Logger.LogWarning("v3.3: AI_Steal tooHeavy 내부 메서드를 찾지 못해 중량 우회를 건너뜁니다.");
    }
    ...
    static bool CallsGetter(MethodBase m){if(Getter==null)return false;try{var il=m.GetMethodBody()?.GetILAsByteArray();if(il==null)return false;int token=Getter.MetadataToken;for(int i=0;i+4<il.Length;i++){byte op=il[i];if(op!=0x28&&op!=0x6f)continue;if(BitConverter.ToInt32(il,i+1)==token)return true;}}catch{}return false;}
    static long EffectiveWeight(Card target)
    {
        long w=target==null?0:target.ChildrenAndSelfWeight;if(!Plugin.StealWeight.Value||target==null||EClass.pc==null)return w;
        long limit=(long)EClass.pc.Evalue(281)*200L+(long)EClass.pc.STR*100L+1000L;if(w<=limit||limit<=0)return w;
        int baseChance=Math.Min(Plugin.StealWeightCap.Value,Math.Max(0,Plugin.Luck()/Math.Max(1,Plugin.StealWeightDiv.Value)));if(baseChance<=0)return w;
        int chance=(int)Math.Max(1L,Math.Min(baseChance,(long)baseChance*limit/Math.Max(1L,w)));
        if(!StealAttemptContext.hasRoll){StealAttemptContext.roll10000=EClass.rnd(10000);StealAttemptContext.hasRoll=true;}
        if(StealAttemptContext.roll10000<chance*100){Plugin.I.Logger.LogInfo($"훔치기 중량 운 우회 성공: {w}>{limit}, 이번 시도 확률 {chance}%");return limit;}
        return w;
    }
    static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions,MethodBase __originalMethod)
    {
        int n=0;foreach(var ci in instructions){if(Getter!=null&&ci.operand is MethodInfo m&&m==Getter&&(ci.opcode==OpCodes.Call||ci.opcode==OpCodes.Callvirt)){n++;yield return new CodeInstruction(OpCodes.Call,Helper);}else yield return ci;}
        if(n!=1)Plugin.I.Logger.LogWarning($"v3.3: {__originalMethod.DeclaringType?.Name}.{__originalMethod.Name} 중량 getter 교체 수={n}, 예상=1.");
    }
}
...
static class SeedLuckNarrowPatch
{
    ...
    static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        int count=0;foreach(var ci in instructions){if(ci.operand is MethodInfo m&&m==Rnd&&(ci.opcode==OpCodes.Call||ci.opcode==OpCodes.Callvirt)&&count++==0){yield return new CodeInstruction(OpCodes.Ldarg_1);yield return new CodeInstruction(OpCodes.Call,Helper);}else yield return ci;}
        if(count<2)Plugin.I.Logger.LogWarning($"v3.3 TryPopSeed RNG 패턴 수={count}, 예상>=2.");
    }
}

[HarmonyPatch(typeof(Card),nameof(Card.MakeEgg),new Type[]{typeof(bool),typeof(int),typeof(bool),typeof(int),typeof(BlessedState?)})]
static class FertEggLuckNarrowPatch
{
    static void Prefix(Card __instance,ref int fertChance)
    {
        if(!Plugin.FertEggLuck.Value||__instance==null||!__instance.IsPCFaction||fertChance<=1)return;
        fertChance=Plugin.ReduceDenom(fertChance,Plugin.RelativeBonus(Plugin.EggDiv,Plugin.EggCap));
    }
}
...
{
    static readonly MethodInfo Rnd=typeof(EClass).GetMethod(nameof(EClass.rnd),new Type[]{typeof(int)})!;
    static readonly MethodInfo Helper=typeof(ScratchPrizeNarrowPatch).GetMethod(nameof(ScratchRnd),BindingFlags.Static|BindingFlags.NonPublic)!;
    static IEnumerable<MethodBase> TargetMethods(){foreach(var m in Methods(typeof(TraitCrafter)))if(m.Name.Contains("g__Prize"))yield return m;}
    static IEnumerable<MethodBase> Methods(Type root){foreach(var m in root.GetMethods(BindingFlags.Static|BindingFlags.Instance|BindingFlags.Public|BindingFlags.NonPublic))yield return m;foreach(var t in root.GetNestedTypes(BindingFlags.Public|BindingFlags.NonPublic))foreach(var m in Methods(t))yield return m;}
    ...
    static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions,MethodBase __originalMethod)
    {
        int n=0;foreach(var ci in instructions){if(ci.operand is MethodInfo m&&m==Rnd&&(ci.opcode==OpCodes.Call||ci.opcode==OpCodes.Callvirt)){n++;yield return new CodeInstruction(OpCodes.Call,Helper);}else yield return ci;}
        if(n!=1)Plugin.I.Logger.LogWarning($"v3.3 Scratch Prize RNG 교체 수={n}, 예상=1 ({__originalMethod.Name}).");
    }
}
...
{
    static readonly MethodInfo Rnd=typeof(EClass).GetMethod(nameof(EClass.rnd),new Type[]{typeof(int)})!;
    static readonly MethodInfo Helper=typeof(TreasureRarityNarrowPatch).GetMethod(nameof(TreasureRnd),BindingFlags.Static|BindingFlags.NonPublic)!;
    static IEnumerable<MethodBase> TargetMethods(){foreach(var m in Methods(typeof(ThingGen)))if(m.Name.Contains("g__SetRarity"))yield return m;}
    static IEnumerable<MethodBase> Methods(Type root){foreach(var m in root.GetMethods(BindingFlags.Static|BindingFlags.Instance|BindingFlags.Public|BindingFlags.NonPublic))yield return m;foreach(var t in root.GetNestedTypes(BindingFlags.Public|BindingFlags.NonPublic))foreach(var m in Methods(t))yield return m;}
    ...
    {
        int n=0;foreach(var ci in instructions){if(ci.operand is MethodInfo m&&m==Rnd&&(ci.opcode==OpCodes.Call||ci.opcode==OpCodes.Callvirt)){n++;yield return new CodeInstruction(OpCodes.Call,Helper);}else yield return ci;}
        if(n<2)Plugin.I.Logger.LogWarning($"v3.3 Treasure SetRarity RNG 교체 수={n}, 예상>=2 ({__originalMethod.Name}).");
    }
}
